...
class KeywordSearchViewTests(TestCase):
    ...
    def test_keyword_search_returns_preview_data(self, mock_search_keyword_urls, mock_scrape_leads):
        ...
        self.assertIn("attempted_rows", response.data)
        self.assertIn("duplicate_rows", response.data)
        self.assertIn("create_failed_rows", response.data)
        self.assertIn("scrape_failed_urls_count", response.data)
